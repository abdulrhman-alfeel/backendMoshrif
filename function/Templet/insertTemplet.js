  ...
  return async (req, res) => {
    try {
      ...
      for (const row of data) {
        ...
        const typeRaw   = elements[0];
        const nameRaw   = elements[1];
        const daysRaw   = elements[2];
        const ratioRaw  = elements[3];
      ...
      for (const row of data) {
        const elements = Object.values(row);
        const cleanType  = String(elements[0] ?? "").trim();
        const cleanName  = sanitizeName(elements[1]);
        const daysInt    = parsePositiveInt(elements[2]);
        const ratioNum   = Number(parseRatio0to100(elements[3]) || 0);
