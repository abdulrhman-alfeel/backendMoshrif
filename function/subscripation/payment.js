...
const { UpdateState_company_subscriptions } = require("../../sql/update");
const { Select_table_company_subscriptions_vs2 } = require("../../sql/selected/selected");
...
const Creat_payment = () => {
  return async (req, res) => {
    try {
      ...
      if(company_subscriptions_id === null){
        result = await Subscripation_new(id_subscriptions,project_count,userSession?.IDCompany,res,"inactive");
      } else {
        result =  await Select_table_company_subscriptions_vs2(company_subscriptions_id);
      ...
      // const returnUrl = `${process.env.BASE_URL}/payments/clickpay/return?cart_id=${code_subscription}`;
      ...
      return res.status(500).json({ ok: false, message: "create payment failed", raw });
    }
  };
};
...
const clickpay_validate = () => {
  return async (req, res) => {
    try {
      ...
      if (!tran_ref) return res.status(400).json({ ok: false, message: "tran_ref is required" });
      ...
      return res.status(500).json({ ok: false, message: "validate failed", raw });
    }
  };
};
...
//   return async (req, res) => {
//     const { cart_id, tran_ref } = req.query || {};

//     console.log("RETURN:", { cart_id, tran_ref });
...
//     res.send("Payment finished. You can close this page and return to the app.");
//   };
// };

const clickpay_return = () => {
  return async (req, res) => {
    console.log("HIT RETURN:", req.method, req.originalUrl);
    console.log("RETURN headers content-type:", req.headers["content-type"]);
    ...
    // ClickPay أحيانًا يستخدم camelCase مثل tranRef/cartId
    const cartId = q.cart_id || q.cartId || b.cart_id || b.cartId;
    const tranRef = q.tran_ref || q.tranRef || b.tran_ref || b.tranRef;

    console.log("RETURN parsed:", { cartId, tranRef });

    // لا تحدّث الاشتراك من return وحده إذا ما عندك cartId/tranRef
    // الأفضل تربطها بالـ IPN أو Query API
    if (cartId) await UpdateState_company_subscriptions(cartId);
