const { verificationfromdata, convertArabicToEnglish } = require("../../middleware/Aid");
const {
  insertTablecompany,
...
const insertDataCompany = () => {
  return async (req, res) => {
    try {
      ...
      if (await verificationfromdata([CommercialRegistrationNumber,NameCompany,BuildingNumber,StreetName,NeighborhoodName,PostalCode,City,Country,TaxNumber,Api,PhoneNumber,userName])) {
...
const inseertCompanybrinsh = () => {
  return async (req, res) => {
    try {
      ...
      if (NameSub?.length > 0 && BranchAddress?.length > 0) {
