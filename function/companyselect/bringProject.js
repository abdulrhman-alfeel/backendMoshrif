...
const BringProject = () => {
  return async (req, res) => {
    try {
      ...
      const key = `projects:${PhoneNumber}:${IDcompanySub}:${parseInt(IDfinlty)}`;
      
      const cached = await redis.get(key);
      if (cached) {
        const cachedData = JSON.parse(cached);
        // console.log("Data fetched from cache");
        return res.send({ success: true, data: cachedData?.data, boss: cachedData?.boss }).status(200);
      }

      const projects = await getProjectsForUser(PhoneNumber, IDcompanySub, IDfinlty);
